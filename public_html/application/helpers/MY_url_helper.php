<?php
    ...
    function linkCSS ($files_arr, $prefix_url = false, $caching_free = false) {
        ...
        if(false === $prefix_url){
            ...
            foreach($files_arr as $f){
                echo '<link rel="stylesheet" href="' . base_url($prefix_url . $f . '.css'). assets_ver_str($caching_free) . '">'.PHP_EOL;
            }
        }
    }

    function linkJS ($js_files_arr, $prefix_url = false, $caching_free = false) {
        ...
        if(false === $prefix_url){
            ...
            foreach($js_files_arr as $f){
                echo '<script src="' . base_url($prefix_url . $f . '.js'). assets_ver_str($caching_free) . '"></script>'.PHP_EOL;
            }
        }
    }
    ...
    function load_ui_library(array $lib_arr){
        ...
        foreach($lib_arr as $libs){
            ...
            if(array_key_exists($filter_lib_name, $listed_library)){
                if(true===isset($listed_library[$filter_lib_name]['files']['css']))
                    linkCSS($listed_library[$filter_lib_name]['files']['css'], $listed_library[$filter_lib_name]['root_path'], true);
                if(true===isset($listed_library[$filter_lib_name]['files']['js']))
